...
| **Python** | `github.com/JackyZhang8/magicqueue-py`（包 `magicqueue`） | `pip install magicqueue`（Redis：`pip install "magicqueue[redis]"`） | 内置 | [`redis-py`](https://github.com/redis/redis-py) | LevelDB / `sqlite3`                            | `threading` + `queue.Queue` |

> 持久化后端按各语言生态选择无 C 依赖的方案：Go→LevelDB、Rust→sled、Python→LevelDB/sqlite3。
